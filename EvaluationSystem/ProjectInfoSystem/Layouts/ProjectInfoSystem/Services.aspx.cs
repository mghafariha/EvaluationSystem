...
namespace ProjectInfoSystem.Layouts.ProjectInfoSystem
{
    ...
    [WebMethod]
    public static List<WeeklyOperationItem> GetPlanDetailPrevius(int contractId, string startDate,string endDate,string commiteType)
    {
        List<WeeklyOperationItem> list = new List<WeeklyOperationItem>();
        SPWeb web = SPContext.Current.Web;
        SPList list2 = web.GetList("/Lists/PeriodWeeklies");
        SPList list3 = web.GetList("/Lists/WeeklyPlanConstructionsDetails");
        SPQuery query = new SPQuery
        {
            Query = string.Format(@"<Where>
                                        <Lt>
             ...
             SPQuery query2 = new SPQuery
            {
                Query = string.Format(@"<Where>
                                         <And>
                                              <And>
                                                  <Lt>           
                                                     <FieldRef Name='_x062f__x0648__x0631__x0647__x00' />
                                                     <Value Type='DateTime'>{0}</Value>
                                                  </Lt>
                                                   <Geq>           
                                                     <FieldRef Name='_x062f__x0648__x0631__x0647__x00' />
                                                ...
                                                <Eq>
                                                  <FieldRef Name='WeeklyConstruction_x003a__x0646_' />
                                                  <Value Type='Text'>{2}</Value>
                                                </Eq>
                                                </And>
                                               </Where>", SPUtility.CreateISO8601DateTimeFromSystemDateTime(Convert.ToDateTime(startDate)), SPUtility.CreateISO8601DateTimeFromSystemDateTime(Convert.ToDateTime(endDate)),commiteType),
                ViewAttributes = "Scope='RecursiveAll'"
            };
    ...
    public static bool IsExitSameWeeklyPlan(int contractId, int periodId)
    {
        ...
        SPSecurity.RunWithElevatedPrivileges(delegate {
            using (SPSite site = new SPSite(siteId))
            {
                using (SPWeb Web = site.OpenWeb())
                {
                    SPList list = Web.GetList("/Lists/Periods");
    ...
    public static string SaveWeeklyReport(WeeklyOperation weeklyItem, bool isTemp)
    {
     ...
     [WebMethod]
    public static string SaveWeeklyPlan(WeeklyOperation weeklyItem,string commiteType)
    {
        int lookupId;
        SPSecurity.CodeToRunElevated secureCode = null;
        string message = "";
        ...
        item["Title"] = string.Concat(new object[] { " گزارش اجرای پیمان", weeklyItem.Contract, " دوره ", weeklyItem.Period });
        item["Contract"] = new SPFieldLookupValue(weeklyItem.ContractId, "");
        item["TypeCommitment"] = commiteType;
        item["Period"] = new SPFieldLookupValue(weeklyItem.Period, "");
     ...
     [WebMethod]
     public static string SaveFixItem(FixItem fixItem)
     {
         string msg = "";
          SPSecurity.CodeToRunElevated secureCode = null;
         SPWeb web = SPContext.Current.Web;
         ...
         SPList list2 = web.GetList("/Lists/FormPermissions");
         SPListItem itemById = web.GetList("/Lists/Contracts").GetItemById(fixItem.ContractId);
         SPQuery query = new SPQuery
         {
             Query = string.Format(@"<Where>
                                     <Eq><FieldRef Name='ListName' />
                                       <Value Type='Text'>DefectFailures</Value>
                                   </Eq>
                                    </Where>")
         };
         SPListItem item2 = (list2.GetItems(query).Count > 0) ? list2.GetItems(query)[0] : null;
         if (((item2 == null) || (item2["Creator"] == null)) || (item2["Approver1"] == null))
         {
             return (" برای " + itemById["Title"].ToString() + " اطلاعات دسترسی  تکمیل نشده است ");
         }
         int relatedUser = Utility.GetRelatedUser(new SPFieldLookupValue(item2["Creator"].ToString()).LookupId, fixItem.ContractId);
         int userId = Utility.GetRelatedUser(new SPFieldLookupValue(item2["Approver1"].ToString()).LookupId, fixItem.ContractId);
         int num3 = (item2["Approver2"] != null) ? Utility.GetRelatedUser(new SPFieldLookupValue(item2["Approver2"].ToString()).LookupId, fixItem.ContractId) : 0;
         int num4 = (item2["Approver3"] != null) ? Utility.GetRelatedUser(new SPFieldLookupValue(item2["Approver3"].ToString()).LookupId, fixItem.ContractId) : 0;
         List<int> viewersIds = new List<int>();
         SPFieldUserValueCollection values = (itemById["Viewers"] != null) ? new SPFieldUserValueCollection(web, itemById["Viewers"].ToString()) : null;
         if (values != null)
         {
         ...
         SPFolder folder = web.GetFolder("/Lists/DefectFailures/" + fixItem.ContractId.ToString());
         int navigatorGroupId = 0;
         int navigator_editorId = 0;
         int ejraGroupId = 0;
         if (!folder.Exists)
         {
             SPListItem item3 = list.Items.Add(list.RootFolder.ServerRelativeUrl, SPFileSystemObjectType.Folder);
             item3["Title"] = fixItem.ContractId.ToString();
             web.AllowUnsafeUpdates = true;
             item3.Update();
             folder = web.GetFolder("/Lists/DefectFailures/" + fixItem.ContractId.ToString());
             if (secureCode == null)
             {
                 secureCode = delegate
                 {
         ...
         int result = 0;
         SPListItem item = (fixItem.Id > 0) ? list.GetItemById(fixItem.Id) : list.AddItem(folder.ServerRelativeUrl, SPFileSystemObjectType.File);
         item["Title"] = string.Concat(new object[] { " گزارش اجرای پیمان", fixItem.Contract, " دوره ", fixItem.Period });
         item["Contract"] = new SPFieldLookupValue(fixItem.ContractId, "");
         item["TemporaryDelivery"] = new SPFieldLookupValue(fixItem.TemporaryDeliveryId, "");
         item["Period"] = new SPFieldLookupValue(fixItem.Period, "");
         item["Status"] =  "در انتظار تایید";
         item["CurrentUser"] = new SPFieldUserValue(web, userId.ToString());
         item["EquippedDeliveryLevel"]=fixItem.EquippedDeliveryLevel;
          item["DrainageDeliveryLevel"]=fixItem.DrainageDeliveryLevel;
          item["NetDeliveryLevel"]=fixItem.NetDeliveryLevel;
          item["NetDefectDeliminationLevel"]=fixItem.NetDefectDeliminationLevel;
          item["EquippedDefectDeliminationLevel"]=fixItem.EquippedDefectDeliminationLevel;
          item["DrainageDefectDeliminationLevel"]=fixItem.DrainageDefectDeliminationLevel;
          item["NetDeadLineLevel"]=fixItem.NetDeadLineLevel;
          item["EquippedDeadLineLevel"]=fixItem.EquippedDeadLineLevel;
          item["DrainageDeadLineLevel"]=fixItem.DrainageDeadLineLevel;
          item["NetReminedLevel"]=fixItem.NetReminedLevel;
          item["EquippedReminedLevel"]=fixItem.EquippedReminedLevel;
          item["DrainageReminedLevel"] = fixItem.DrainageReminedLevel;
         ...
         if (msg == "")
         {
             string s = SaveDefectFailuresItems(item.ID, fixItem.ContractId, new SPFieldLookupValue(item["Period"].ToString()).LookupValue, new SPFieldLookupValue(item["Period"].ToString()).LookupId, fixItem.Items, relatedUser, userId, num3, num4, viewersIds, editorsIds);
             int.TryParse(s, out result);
             if (result < 0)
             {
                 return s;
             }
             string str3 = "/_Layouts/15/ProjectInfoSystem/Pages/Operation/EditForm.aspx?ListName=DefectFailures&ID=" + item.ID;
            
             if (msg == "")
             {
                 msg = Utility.CreateHistory(web, item.ID, DateTime.Now.ToString(), "ثبت", "", "DefectFailures", web.CurrentUser.ID);
             }
         }
         return msg;
     }
     ...
     private static string SaveDefectFailuresItems(int id, int contractId, string period, int periodId, List<FixItemDetail> items, int creatorId, int approver1Id, int approver2Id, int approver3Id, List<int> viewersIds, List<int> editorsIds)
     {
         string msg = "";
         SPWeb web = SPContext.Current.Web;
         Guid siteID = web.Site.ID;
         SPSecurity.RunWithElevatedPrivileges(delegate
         {
             using (SPSite site = new SPSite(siteID))
             {
                 using (SPWeb Web = site.OpenWeb())
                 {
                     SPList list = Web.GetList("/Lists/DefectFailuresDetails");
                     Web.AllowUnsafeUpdates = true;
                     SPFolder folder = Web.GetFolder("/Lists/DefectFailuresDetails/" + contractId.ToString());
                     if (!folder.Exists)
                     {
                         SPListItem item = list.Items.Add(list.RootFolder.ServerRelativeUrl, SPFileSystemObjectType.Folder);
                         item["Title"] = contractId.ToString();
                         Web.AllowUnsafeUpdates = true;
                         item.Update();
                         folder = Web.GetFolder("/Lists/DefectFailuresDetails/" + contractId.ToString());
                         Utility.SetListItemPermission(folder.Item, creatorId, 0x40000002, true);
                         Utility.SetListItemPermission(folder.Item, approver1Id, 0x40000002, false);
                         if (approver2Id != 0)
                         {
                     ...
                     SPFolder folder2 = Web.GetFolder("/Lists/DefectFailuresDetails/" + contractId.ToString());
                     SPQuery query = new SPQuery
                     {
                         Query = string.Format(@"<Where>
                                              <Eq>
                                                 <FieldRef Name='DefectFailure' LookupId='TRUE' />
                                                 <Value Type='Lookup'>{0}</Value>
                                              </Eq>
                                           </Where>", id),
                         Folder = folder2
                     ...
                     foreach (FixItemDetail item3 in items)
                     {
                         SPListItem item4 = list.AddItem(folder.ServerRelativeUrl, SPFileSystemObjectType.File);
                         item4["DefectFailure"] = new SPFieldLookupValue(id, "");
                         item4["NetLevel"] = item3.NetLevel;
                         item4["DrainageLevel"] = item3.DrainageLevel;
                         item4["EquippedLevel"] = item3.EquippedLevel;
                         item4["Reason"] = new SPFieldLookupValue(item3.ReasonId, "");
                         item4["Description"] = item3.Description;


                         item4["Title"] = "جزییات رفع نقص ";
                      //   item4["Period"] = new SPFieldLookupValue(periodId, "");
                         try
                         {
                             Web.AllowUnsafeUpdates = true;
