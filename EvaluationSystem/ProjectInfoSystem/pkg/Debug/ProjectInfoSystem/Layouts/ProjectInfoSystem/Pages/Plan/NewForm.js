...
app.controller('plnCtrl', function ($scope, service, $q) {
    ...
    $scope.commiteTypes = ['تعهد اولیه', 'تعهد بازنگری'];
    ...
    $scope.showOperations = function () {
        ...
        service.isExitSameWeeklyPlanReport($scope.contract.Id, $scope.period.Id).then(function (result) {
            ...
            if ($scope.isExitSameReport) {
                ...
                $q.all([service.getOperationsOnFarmsFilterContract($scope.contract.Id), service.getWeeklyDetailPrevius($scope.contract.Id, moment('1397/01/01', 'jYYYY/jMM/jDD').format('YYYY/MM/DD')), service.getWeeklyPlanDetailPrevius($scope.contract.Id, moment($scope.period.StartDate).format('YYYY/MM/DD'), moment('1397/01/01', 'jYYYY/jMM/jDD').format('YYYY/MM/DD'),$scope.commiteType)]).then(function (a) {
                    $scope.tableData = a[0].data.d.results;
                    $scope.priviousDetails = a[1].data.d;
                    $scope.priviousPlan = a[2].data.d;
                    // calcTotal();
    ...
    $scope.save = function (isTemp) {
        ...
        service.savePlan(weeklyItem,$scope.commiteType).then(function (d) {
            $scope.loading = false;
            if (d.data.d == "") {
